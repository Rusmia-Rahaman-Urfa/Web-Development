...
$mobileProducts = []; // Initialize an empty array to store products fetched from DB
// IMPORTANT: Ensure this EXACTLY matches the category name from your admin panel
$targetCategory = 'Digital Mobile Phones'; 
...
if ($stmt = $conn->prepare($sql)) {
    ...
    $mobileProducts = []; // Ensure it's empty to prevent display issues
}
...
<h1>Digital Mobile Phones</h1>

<div class="product-detail-container">
    <?php if (!empty($mobileProducts)): ?>
        <?php foreach ($mobileProducts as $p): ?>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <p>No digital mobile phones found in this category yet. Please add some from the admin panel.</p>
    <?php endif; ?>
</div>
